...
class OtherBadges : Plugin() {
    private val badgeMap = mutableMapOf<Long, MutableList<BadgeData>>()
    ...
    override fun start(context: Context) {
        ...
        patcher.after<UserProfileHeaderView>(
            ...
            // Remove all our badges first to prevent duplicates
            data.removeAll { it.objectType?.startsWith("http") == true }
            
            // Add custom badges
            val customBadges = userBadges.map { 
                Badge(0, null, it.tooltip, false, it.badge) 
            }
        ...
        patcher.after<UserProfileHeaderView.BadgeViewHolder>("bind", Badge::class.java) {
            ...
            // Check that badge is ours (has our marker)
            if (url?.startsWith("http") != true) return@after
    ...
    private fun fetchBadges() {
        ...
        Utils.threadPool.execute {
            val tempBadges = mutableMapOf<Long, MutableList<BadgeData>>()
            ...
            for (source in sources) {
                try {
                    ...
                    while (keys.hasNext()) {
                        ...
                        val badges = tempBadges.getOrPut(userId) { mutableListOf() }
                        ...
                        for (i in 0 until badgesArray.length()) {
                            ...
                            badges.add(BadgeData(tooltip, badgeUrl))
                        }
                    }
                } catch (e: Exception) {
